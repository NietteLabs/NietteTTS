...
cat userdict.tsv >> models/train.dic 
...
LD_LIBRARY_PATH=../tools/g2p/lib/$ARCH/ phonetisaurus-align --input=models/train.dic --ofile=models/train.corpus --seq1_del=false --seq2_del=false --nbest=8 --mbr=true --iter=40 --grow=true --fb=true
echo "Treinando modelo n-gram (5s-10s):" 
estimate-ngram -wv vocab -o 6 -t models/train.corpus -wl /tmp/ngram.tmp
rm /tmp/ngram.tmp
lmplz --discount_fallback --collapse_values -o 6 --limit_vocab_file vocab < models/train.corpus > models/train.o8.arpa
